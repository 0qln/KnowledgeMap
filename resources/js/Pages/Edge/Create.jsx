...
function Edge({ }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('dashboard.edges.store'));
    }

    return (
        <div>
            <Head title="Create Edge" />
            ...
            <form onSubmit={submit} className="flex flex-row-reverse">
                ...
                <div className="flex flex-col w-full">
                    ...
                    <div className="flex flex-col w-full space-y-6 mt-5">
                        <div>
                            <InputLabel value="Source Id" />
                            <TextInput
                                className="block w-full"
                                value={data.id_origin}
                                type="number"
                                id="origin"
                                onChange={(e) => setData('id_origin', e.target.value)}
                                autoComplete="off"
                                required />
                            <InputError className="mt-2" message={errors.id_origin} />
                        </div>
                        <div>
                            <InputLabel value="Target Id" />
                            <TextInput
                                className="block w-full"
                                value={data.id_target}
                                type="number"
                                id="target"
                                onChange={(e) => setData('id_target', e.target.value)}
                                autoComplete="off"
                                required />
                            <InputError className="mt-2" message={errors.id_target} />
                        </div>
                        <div>
